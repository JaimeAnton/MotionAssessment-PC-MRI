...
%% Cropping image (ROI) - Manual segmentation
% %crop_roi=zeros(11,4);%import the ROI from file
% crop_roi=readmatrix('Cropped_ROIs_.xls');
% 
% %Modify this line to choose the dataset to analyse
% image_index=10;
% 
% figure(image_index)
% imshow(M{image_index}(:, :, :,1),[])
% r = drawrectangle();
% roi_crop=r.Position;
% 
% %Saving ROIs
% crop_roi(image_index,:)=roi_crop;
% writematrix(crop_roi,'Cropped_ROIs_.xls')

%% Segmentation of the prostate only (Mask)
% %Modify this line to choose the dataset to analyse
% image_index=10;
% 
% %Get the cropped image
% crop=readmatrix('Cropped_ROIs_.xls');
% roi_crop=crop(image_index,:);
% x=roi_crop(1,1);
% y=roi_crop(1,2);
% width=roi_crop(1,3);
% height=roi_crop(1,4);
% croped_image = mat2gray(M{image_index}(y:y+height-1, x:x+width-1, :,:));
% I_croped =croped_image(:, :, :,1);
% 
% roi_poly=[];
% i_min = min_max(1, 1);
% i_max = min_max(1, 2);
% entropy_matrix=zeros(i_max-i_min+1,1);
% my_entropy_matrix=zeros(i_max-i_min+1,1);
% h=size(croped_image);
% for index_roi=1:h(4)
%     image=croped_image(:, :, :,index_roi);
%     figure(15)
%     imshow(image,[])
%     r = drawpolygon();
%     roi_poly=r.Position;
%     % Create mask from circle coordinates.
%     [rows, columns, numColorChannels] = size(image);
%     irisMask = poly2mask(roi_poly(:,1), roi_poly(:,2), rows, columns);
%     % Mask outside the circle to black, 0.
%     image(~irisMask) = 0;
%     figure(15)
%     imshow(image,[])
%     entropy_matrix(index_roi) = entropy(image(irisMask));
%     my_entropy_matrix(index_roi) = my_entropy(image(irisMask));
%     filename=strcat('Manual_segmentation_ROIs_', num2str(image_index), '.xls');
%     writematrix(roi_poly,filename,'Sheet',index_roi);
% end

%% Entropy calculations (using only previously obtained ROIs)
% entropy_matrix=zeros(i_max-i_min+1,1);
% my_entropy_matrix=zeros(i_max-i_min+1,1);
% h=size(croped_image);
% for index_roi=1:h(4)
%     image=croped_image(:, :, :,index_roi);
%     roi_poly=readmatrix('Manual_segmentation_ROIs_10.xls','Sheet',index_roi);
...
%     N = nnz(image(irisMask));
% 
%     entropy_matrix(index_roi) = entropy(image(irisMask)/N);
%     my_entropy_matrix(index_roi) = my_entropy(image(irisMask)/N);
%     filename=strcat('Manual_segmentation_ROIs_', num2str(image_index), '.xls');
%     writematrix(roi_poly,filename,'Sheet',index_roi);
% end
% 
% %% Plotting figure entropy
% i_min = min_max(image_index, 1);
% i_max = min_max(image_index, 2);
% figure(16)
% plot(i_min:i_max,my_entropy_matrix)
% hold on;
% title('Plot of Entropy for each slice 10')
% 
